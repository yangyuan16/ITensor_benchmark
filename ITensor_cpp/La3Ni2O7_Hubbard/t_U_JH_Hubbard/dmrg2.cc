...
int main(int argc, char* argv[])
{
	...
	string pathname = "Output/80/5000";
	string pathnameIN = "Output/3000";
		
	auto sites = Electron(N, { "ConserveQNs =", true });
	readFromFile(pathnameIN + "sites_file", sites);
	itensor::MPS psi0(sites);
	readFromFile(pathnameIN + "psi_file", psi0);
	...
	auto [energy, psi] = dmrg(H, psi0, sweeps, { "Quiet=",true, "WriteDim", 6000, "WriteDir", "Output/PH" });
	...
	printf(" The value of Nt is in "+pathname+" Ntot.txt. \n");
	std::stringstream ss;
	...
	printf(" The value of Fzz is in "+pathname+" Fzz.txt.\n");
	ss.str("");
	ss << pathname << "Fzz.txt";
	filename = ss.str();
	fout.precision(12);
	...
	for (int j = jsta; j <= jend; j++){
		...
		for (int r = jsta; r <= jend; r++) {
			...
			auto Pzz = toMPO(PCMz);
			fout << log10(fabs(inner(psi, Pzz, psi))) << "\t";
			}
		fout << std::endl;
	}
	fout.close();
        printf(" The value of Pzz is in "+pathname+" Pzz.txt.\n");
	...
	fout.close();
	printf(" The value of Pxy is in "+pathname+" Pxy.txt.\n");
